...
('Departamento 27', '2025-03-25', 50),
('Departamento 28', '2025-04-30', 50),
('Departamento 29', '2025-05-02', 50),
('Departamento 30', '2025-06-15', 30),
('Departamento 31', '2025-07-22', 31),
('Departamento 32', '2025-08-11', 32),
('Departamento 33', '2025-09-09', 49),
('Departamento 34', '2025-10-28', 49),
('Departamento 35', '2025-11-15', 49),
('Departamento 36', '2025-12-10', 49),
('Departamento 37', '2025-12-10', 49),
('Departamento 38', '2025-12-10', 50);
...
WHERE c.nome NOT IN ('Lisboa', 'Porto');


CREATE VIEW v_empregados_departamentos AS
SELECT 
    d.cod_departamento,
...
GROUP BY 
 ...
 SELECT * FROM v_empregados_departamentos;
...
CREATE OR REPLACE PROCEDURE proc_arquivar_projeto(cod_projeto_param INT)
LANGUAGE plpgsql
AS $$
BEGIN
    -- Passo 1: Inserir o projeto na tabela tbl_projetos_arquivados
    INSERT INTO tbl_projetos_arquivados (cod_projeto, nome, cod_departamento)
    SELECT cod_projeto, nome, cod_departamento
    FROM tbl_projetos
    WHERE cod_projeto = cod_projeto_param;
    
    -- Passo 2: Excluir o projeto da tabela tbl_projetos
    DELETE FROM tbl_projetos
    WHERE cod_projeto = cod_projeto_param;
    
    RAISE NOTICE 'Projeto % arquivado com sucesso!', cod_projeto_param;
END;
$$;

CALL proc_arquivar_projeto(1);



CREATE OR REPLACE PROCEDURE proc_arquivar_projeto(
    cod_projeto_param INT,
...
DECLARE
    sql_insert TEXT;
BEGIN
    sql_insert := format('INSERT INTO %I (cod_projeto, nome, cod_departamento) 
                          SELECT cod_projeto, nome, cod_departamento 
                          FROM tbl_projetos 
                          WHERE cod_projeto = $1', nome_tabela_destino);
    EXECUTE sql_insert USING cod_projeto_param;

    DELETE FROM tbl_projetos
    WHERE cod_projeto = cod_projeto_param;
    RAISE NOTICE 'Projeto % movido para a tabela % e excluído de tbl_projetos', 
                 cod_projeto_param, nome_tabela_destino;
END;
$$;

CALL proc_arquivar_projeto(1, 'tbl_projetos_arquivados');

CALL proc_arquivar_projeto(2, 'tbl_projetos_arquivados');


CREATE OR REPLACE PROCEDURE proc_atualiza_cidade_projeto(
    cod_projeto INT,
    cod_cidade INT
)
LANGUAGE plpgsql
...
BEGIN
    ...
    RAISE NOTICE 'Cidade do departamento associado ao projeto % foi atualizada para %.', cod_projeto, cod_cidade;
END;
$$;

CALL proc_atualiza_cidade_projeto(1, 5);