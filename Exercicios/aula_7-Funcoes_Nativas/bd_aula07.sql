...
-- DROP DATABASE IF EXISTS bd_aula07;
...
SELECT UPPER(f.nome) AS nome_fornecedor, LOWER(p.nome) AS nome_peca FROM tbl_fornecedor f
JOIN tbl_estoque e ON f.cod_fornecedor = e.cod_fornecedor
JOIN tbl_peca p ON e.cod_peca = p.cod_peca
WHERE f.cidade = p.cidade
ORDER BY nome_fornecedor, nome_peca;
...
SELECT p.nome AS nome_peca, p.cor AS cor_peca FROM tbl_peca p
...
SELECT f.nome AS nome_fornecedor, f.cidade AS cidade_fornecedor FROM tbl_fornecedor f
JOIN tbl_estoque e ON f.cod_fornecedor = e.cod_fornecedor
WHERE e.cod_peca = 1
...
SELECT SUM(e.quantidade) AS quantidade_total FROM tbl_estoque e
WHERE e.cod_peca = 1;

--6 Listar a média dos preços das peças fornecidas pelo fornecedor com código 3.
SELECT AVG(p.preco) AS media_precos FROM tbl_peca p
JOIN tbl_estoque e ON p.cod_peca = e.cod_peca
WHERE e.cod_fornecedor = 3;
...
SELECT COUNT(*) AS quantidade_pecas
FROM tbl_peca;

--9 listar a quantidade de cidades diferentes onde existem peças cadastradas
SELECT COUNT(DISTINCT cidade) AS quantidade_cidades_diferentes
FROM tbl_peca;
