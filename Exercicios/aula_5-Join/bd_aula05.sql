...
SELECT t.titulo, COALESCE(l.status, 'NÃO DEFINIDO') AS status
FROM tbl_titulo t
...
LEFT JOIN tbl_livros l ON e.codigo_livro = l.cod_livro AND l.status = 'ALUGADO'
...
SELECT c.nome AS cliente_nome, t.titulo AS livro_titulo
FROM tbl_cliente c
...
SELECT c.nome AS cliente_nome, t.titulo AS livro_titulo, l.status AS livro_status
FROM tbl_cliente c
JOIN tbl_emprestimo e ON c.codigo_cliente = e.codigo_cliente
JOIN tbl_livros l ON e.codigo_livro = l.cod_livro
JOIN tbl_titulo t ON l.codigo_titulo = t.codigo_titulo
