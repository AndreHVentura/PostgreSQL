...
CREATE TRIGGER trg_atualizar_qtde_projetos
...
--exercicio 2--
CREATE OR REPLACE FUNCTION atualizar_qtde_projetos_insercao()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE tbl_departamentos
    SET qtde_projetos = qtde_projetos + 1
...
CREATE TRIGGER trg_atualizar_qtde_projetos_insercao
AFTER INSERT ON tbl_projetos
FOR EACH ROW
EXECUTE FUNCTION atualizar_qtde_projetos_insercao();

INSERT INTO tbl_projetos (nome, cod_departamento)
VALUES ('Projeto DC', 12);
...
VALUES (1, 5000.00, '2024-01-01'),
       (2, 5200.00, '2024-02-01'),
       (3, 4800.00, '2024-03-01');

SELECT e.nome, h.salario, h.data
FROM tbl_empregados e
JOIN tbl_empregado_historico_salario h ON e.cod_empregado = h.cod_empregado
ORDER BY e.nome, h.data;

--exercicio 3 B--
CREATE OR REPLACE FUNCTION fn_salvar_historico_salario()
RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO tbl_empregado_historico_salario (cod_empregado, salario, data)
...
CREATE TRIGGER trg_salvar_historico_salario
AFTER UPDATE OF salario ON tbl_empregados
FOR EACH ROW
EXECUTE FUNCTION fn_salvar_historico_salario();
...
SELECT cod_empregado, nome, data_atualizacao
FROM tbl_empregados
WHERE cod_empregado = 1;
